...
import fallbackAppsData from "../data/fallbackApps.json";
...
const fallbackApps: AppInfo[] = fallbackAppsData as AppInfo[];
...
export const useWebOS = () => {
	...
	useEffect(() => {
		...
		const runBoot = async () => {
			...
			let sysconfigParsed: Record<string, unknown> | null = null;
			...
			try {
				...
				if (sysconfigNode && sysconfigNode.content) {
					...
					if (sysconfigParsed && sysconfigParsed["is_configured"]) {
						isConfigured = true;
					}
				}
			} catch (err) {
